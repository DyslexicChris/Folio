 ...
 *
 * @param maxBodySizeMB {number} Max body size in MB to configure the middleware produced from the factory with.
 ...
 * @constructor
 */
function JSONBodyParserFactory(maxBodySizeMB) {
    'use strict';

    this._maxBodySize = this._maxBodySizeBytes(maxBodySizeMB);
}
 ...
 * @returns {Number}
 */
JSONBodyParserFactory.prototype.maxBodySize = function () {
    'use strict';
...
JSONBodyParserFactory.prototype._handleTooLargeRequest = function (request, response) {
    ...
    response.statusCode = HTTPConstants.statusCodes.REQUEST_TOO_LARGE;
    response.end();
